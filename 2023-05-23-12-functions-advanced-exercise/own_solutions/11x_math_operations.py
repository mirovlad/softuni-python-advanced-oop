# Avoids using % to find the right key by emulating it by incrementing key_idx
# A few more lines but a tiny bit cheaper
def math_operations(*args, **kwargs):
    ...
    key_idx = -1
    for arg in args:
        # Emulate "% keys_len" using += 1 and correction if exceeds the highest index in keys
        # Feels cheaper than "% keys_len"
        key_idx += 1
        if key_idx == keys_len:
            key_idx = 0
        key = keys[key_idx]
